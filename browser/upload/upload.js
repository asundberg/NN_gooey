...
app.controller('UploadCtrl', function($rootScope, $scope, TestingFactory, TrainerFactory, DatasetFactory, $state, $timeout) {
  ...
  $scope.chooseAvailableDataset = function (dataset) {
    var datasetLink;
    if (dataset.name === 'Iris dataset') {
      datasetLink = '/iris.txt';
    } else if (dataset.name === 'Humor types') {
      datasetLink = '/preprocessed-humordata.csv';
    } else if (dataset.name === 'Soybean types') {
      datasetLink = '/soybean-small.data.txt';
    } else {
      datasetLink = '/Machiavellian/MachThousandWithHeader.csv';
    }
    DatasetFactory.getDataset(datasetLink)
    .then(function (response) {
      convertToArr(response);
      $scope.showData = true;
      // $scope.$digest();
    });
  };

  // END OF DATASET STUFF
